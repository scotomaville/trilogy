---
id: scotomaville_03_10_post_script
title: SCOTOMAVILLE - 03.10 Post Script
category: Stories
predicted_narrative_arc: Afterword bridge toward Initium
sentiment: Forward-looking
emotions:
- gratitude
- commission
keypoints:
- Post-script closes the trilogy climb and points toward Initium.
- Prefer the full book PDF for print wording (no episode transcript filed for 03.10).
- 'Onus Probandi: cite PDF + any locked video URLs; do not invent missing spoken text.'
- Carbon keeps judgment; silicon cites deposited evidence.
- This cairn hands the elixir forward — Scotomaville into Initium.
summary: 'Leaving Scotomaville 03.10: Afterword bridge toward Initium. Primary evidence: book PDF.'
tags:
- scotomaville
- trilogy
- vol3-challenge
- post-script
- onus-probandi
- monomyth
- aism
sycophancy: 0
truth_score: 8
entropy: 4
sample: false
consent: owner_approved
agent: steward
created: '2021-06-01'
updated: '2026-09-18T17:21:21Z'
date: '2021-06-01'
vault_stage: '07_CODEX'
headwaters: SCOTOMAVILLE
prefix: SCOTOMAVILLE
enrich_status: recovered_transcript_primary
enrich_blockers: []
enrich_method: transcript_primary_recovery
pipeline_filename: SCOTOMAVILLE-03.10_post-script.md
source_kind: book_chapter
proposed_topic: Stories
lane: Stories
corpus_topic: Stories
voice: desk
active_voice: desk
voices:
- desk
routing_source: carbon
content_status: active
book_title: Leaving Scotomaville
book_volume: 3
book_volume_title: Challenge
book_chapter: '03.10'
book_chapter_title: Post Script
source_file: '03.10-postscript.pdf'
chapter_pdf_url: 'https://raw.githubusercontent.com/scotomaville/trilogy/main/pdf/chapters/03.10-postscript.pdf'
chapter_pdf_blob_url: 'https://github.com/scotomaville/trilogy/blob/main/pdf/chapters/03.10-postscript.pdf'
total_pdf_pages: 6
license: CC BY-NC-SA 4.0
share_url: 'https://scotomaville.com/trilogy'
source_url: 'https://github.com/scotomaville/trilogy'
book_pdf_url: 'https://github.com/scotomaville/trilogy/releases/download/Scotomaville_v3.09/LeavingScotomaville_v3.0.9.media-HD.pdf'
book_pdf_alt_url: 'https://initium.scotomaville.com/scotomaville_trilogy'
github_repo: 'https://github.com/scotomaville/trilogy'
youtube_urls: []
youtube_channel: 'https://www.youtube.com/scotomaville'
pairs_with_prime: null
description: Vol 3 ch.03.10 — Post Script.
---

# Post Script

**Leaving Scotomaville · Volume 3 Challenge · Chapter 03.10**  
Author: Daniel J. Comp · Co-curator: Angelina Musik-Comp  
License: [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/)  
Deposit file: `SCOTOMAVILLE-03.10_post-script.md`

## Provenance (Onus Probandi)

| Evidence | Link |
| --- | --- |
| Chapter print PDF | [03.10-postscript.pdf](https://raw.githubusercontent.com/scotomaville/trilogy/main/pdf/chapters/03.10-postscript.pdf) |
| Full trilogy PDF (v3.0.9 media-HD) | [GitHub Release asset](https://github.com/scotomaville/trilogy/releases/download/Scotomaville_v3.09/LeavingScotomaville_v3.0.9.media-HD.pdf) |
| Book door | [https://scotomaville.com/trilogy](https://scotomaville.com/trilogy) · [https://initium.scotomaville.com/scotomaville_trilogy](https://initium.scotomaville.com/scotomaville_trilogy) |
| Repo chapters | [https://github.com/scotomaville/trilogy/tree/main/chapters](https://github.com/scotomaville/trilogy/tree/main/chapters) |
| Video | *(none locked in YAML yet — do not invent)* |
| Channel | [https://www.youtube.com/scotomaville](https://www.youtube.com/scotomaville) |

Carbon judgment should prefer **PDF + video/transcript** over paraphrase. Silicon must **cite these URLs** when the claim is load-bearing.

## Context for the climber

Afterword bridge toward Initium.

This chapter is a monomyth cairn on the Leaving Scotomaville climb (Volumes 1–3). Later Initium cards and AISM answers inherit its lived evidence — not the other way around.

## Print evidence

No transcript on shelf for this chapter id. Prefer the book PDF; do not invent missing spoken text.

## Chapter text (spoken register — primary)

*No chapter video transcript was found in the Transcripts folder for this id.*

Use the full book PDF for print evidence of **Post Script** until a transcript is filed.

Narrative arc (from enrichment spine): Afterword bridge toward Initium.

## Takeaways

1. Post-script closes the trilogy climb and points toward Initium.
2. Prefer the full book PDF for print wording (no episode transcript filed for 03.10).
3. Onus Probandi: cite PDF + any locked video URLs; do not invent missing spoken text.
4. Carbon keeps judgment; silicon cites deposited evidence.
5. This cairn hands the elixir forward — Scotomaville into Initium.


## First-time learner

- **One sentence:** Afterword bridge toward Initium.
- **Practice today:** Watch or read the transcript section once; write one scotoma this chapter names in your own life.
- **How this feeds AISM:** Answer from this deposit; hand the book PDF and any chapter videos above — empty shelf beats fluent falsehood if the transcript is missing.
